...
2. [Приведение и конвертация типов](#2-приведение-и-конвертация-типов)
3. [Работа со строками](#3-работа-со-строками)
4. [Операторы и управление потоком](#4-операторы-и-управление-потоком)
5. [Классы и объекты](#5-классы-и-объекты)
6. [Наследование и полиморфизм](#6-наследование-и-полиморфизм)
7. [Интерфейсы и DI](#7-интерфейсы-и-di)
8. [Коллекции и LINQ](#8-коллекции-и-linq)
9. [Обобщения (Generics)](#9-обобщения-generics)
10. [Исключения и ресурсы](#10-исключения-и-ресурсы)
11. [Значимые и ссылочные типы](#11-значимые-и-ссылочные-типы)
12. [Null-операторы](#12-null-операторы)
13. [DateTime и TimeSpan](#13-datetime-и-timespan)
14. [Лучшие практики и принципы](#14-лучшие-практики-и-принципы)
...
## 2. Приведение и конвертация типов

### Что помнить

- Неявные преобразования безопасны (например, `int` → `long`), явные могут потерять данные (например, `double` → `int`).
- `Convert.ToXxx` выбрасывает исключения при ошибке, `TryParse` возвращает `bool` и не кидает исключения.
- `checked` ловит переполнение для чисел; по умолчанию арифметика без `checked` в `Release` не проверяет переполнение.
- Боксинг: значение упаковывается в `object`; повторное извлечение требует точного типа (`(int)obj`). Частый боксинг бьет по производительности.
...
// Неявное (расширяющее)
int a = 10;
long b = a; // Ок, без потери

// Явное (сужающее) + checked
double c = 123.45;
int d = checked((int)c); // 123, дробная часть отбрасывается

// Convert vs Parse/TryParse
string s1 = "42";
int n1 = Convert.ToInt32(s1);   // Исключение при null/мусоре
bool ok = int.TryParse(s1, out int n2); // false без исключений

// DateTime безопаснее через TryParseExact при фиксированном формате
...
## 15. ЧЕК-ЛИСТ ИТОГОВОЙ ПРОВЕРКИ ЗНАНИЙ 🎯
