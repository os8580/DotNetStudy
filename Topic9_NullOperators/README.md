...
- **Прочитайте разделы:** "Что такое null?", "?.» (null-conditional), "??" (null-coalescing), "??=" (null-coalescing assignment).
- **Запустите Program.cs** и посмотрите, как каждый оператор предотвращает ошибки: `dotnet run`
...
## 11. ЧЕК-ЛИСТ ДЛЯ СОБЕСЕДОВАНИЯ 🎯

### Вопрос 1: Что такое null и почему это опасно?

**Краткий ответ:** `null` — отсутствие объекта. Доступ к свойству null объекта вызывает `NullReferenceException`. Нужны операторы для безопасной работы.

### Вопрос 2: Что делает оператор `?.` (null-conditional)?

**Краткий ответ:** `?.` проверяет на null и возвращает null если объект null, вместо ошибки. Пример: `user?.Name` безопасен, даже если user null.

### Вопрос 3: Как использовать `??` (null-coalescing)?

**Краткий ответ:** `??` возвращает левое значение, если оно не null, иначе правое. Пример: `name ?? "Unknown"` вернет "Unknown" если name null.

### Вопрос 4: Как работает `??=` (null-coalescing assignment)?

**Краткий ответ:** `??=` присваивает право только если переменная null. Пример: `cache ??= new List()` инициализирует только если cache null.

### Вопрос 5: Что делает `!` (null-forgiving) и когда его использовать?

**Краткий ответ:** `!` "утверждает" компилятору, что значение не null. **РЕДКО** используется и только если вы уверены, что не будет null.

### Вопрос 6: Можно ли комбинировать `?.` и `??`?

**Краткий ответ:** Да, очень полезно. Пример: `page?.User?.Name ?? "Unknown"` — безопасный доступ + значение по умолчанию.

### Вопрос 7: Чем отличается `?.` от обычной проверки `!= null`?

**Краткий ответ:** `?.` компактнее и безопаснее; `!= null` требует явной проверки. Обе работают, но `?.` читается легче.

### Вопрос 8: Как избежать NullReferenceException в коде?

**Краткий ответ:** Используйте `?.`, `??`, `??=`; проверяйте null перед использованием; возвращайте пустые коллекции вместо null.

### Вопрос 9: Какие практические примеры null-операторов в QA-тестах?

**Краткий ответ:** Безопасный доступ к элементам страницы (`element?.Text`), инициализация логов (`logs ??= new List()`), значения конфига по умолчанию, обработка API ответов которые могут быть null.
