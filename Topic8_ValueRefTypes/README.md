...
- Прочитайте: различия Value vs Reference, `ref`/`out`, Nullable (`T?`), нюанс со `string`, базовая потокобезопасность.
- Запустите Program.cs и посмотрите разницу копирования и поведения ссылок.
- Вернитесь к чек‑листу: под каждым вопросом есть краткий ответ для закрепления.
...
Краткий ответ: Значимые (`struct`, `int`) копируются по значению; ссылочные (`class`, `string`) копируются по адресу — разные переменные указывают на один объект.

### Вопрос 2: Что копируется при присваивании?

Краткий ответ: Для значимого типа — само значение; для ссылочного — ссылка (адрес объекта), поэтому изменения видны через обе переменные.

### Вопрос 3: Как сравнивать значения и ссылки?

Краткий ответ: Значимые сравниваются по значениям; ссылочные — по ссылке (`==` по умолчанию сравнивает ссылку). Используйте `ReferenceEquals` для ссылки и переопределяйте равенство по необходимости.

### Вопрос 4: Почему `string` ведет себя «как значимый»?

Краткий ответ: `string` — ссылочный, но иммутабельный: при «изменении» создается новый объект, поэтому старая ссылка не меняется.

### Вопрос 5: Зачем нужны `ref` и `out` и чем отличаются?

Краткий ответ: `ref` — передача существующей переменной по ссылке (должна быть инициализирована); `out` — выходной параметр (инициализируется внутри метода).

### Вопрос 6: Что такое `Nullable` (`T?`) и как использовать?

Краткий ответ: `T?` позволяет значимым типам быть `null`; проверяйте `HasValue`/`!= null` и используйте `??` для значения по умолчанию.

### Вопрос 7: Когда использовать `struct`, а когда `class`?

Краткий ответ: `struct` — для маленьких неизменяемых значений (координаты, диапазоны); `class` — по умолчанию, для объектов с поведением и изменяемым состоянием.

### Вопрос 8: Что такое race condition и как защититься?

Краткий ответ: Гонка — одновременная запись из нескольких потоков; защищайтесь `lock`/`Monitor` или используйте `ConcurrentDictionary`, `ConcurrentQueue` и атомарные операции.

### Вопрос 9: Как избежать `NullReferenceException`?

Краткий ответ: Проверяйте на `null`, используйте `?.`/`??`, проектируйте API так, чтобы возвращать пустые коллекции вместо `null`.
